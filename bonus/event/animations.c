...
void	paint_anim(t_vars *vars, int j)
{
	...
	if (is_exit(vars, vars->assets[FLOOR].img.curr_sx,
			vars->assets[FLOOR].img.curr_sy) == 0)
		paint(vars, &vars->assets[FLOOR], vars->assets[FLOOR].img.curr_sx,
			vars->assets[FLOOR].img.curr_sy);
	where_mov(vars);
	update_p(vars, j);
	paint_canvaw(vars, &vars->assets[vars->pdirection].img, 400, 40);
	mlx_put_image_to_window(vars->mlx, vars->win, vars->canva.img, 0, 0);

...
int     keynotpress(t_vars *vars)
{
     ...
     if(vars->keypress == 0)
    {
        if(i == 0)
            i = 320;
        paintback(vars, &vars->person.img[0], vars->person.curr_sx,vars->person.curr_sy , i + 40, i, width, heigth);
        paintcanva(vars,&vars->img,0,0);
  
        paintcanvaW(vars, &vars->person.img[0] ,vars->person.curr_sx, vars->person.curr_sy , i + 40, i, width, heigth);
